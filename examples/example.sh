...
# 检查工具是否可用
if [ ! -f \"$SCRIPT_DIR/uv_env_manager.sh\" ]; then
echo -e \"${YELLOW}错误: 找不到 uv_env_manager.sh 文件${NC}\"
...
\"$SCRIPT_DIR/uv_env_manager.sh\" create demo-env python3.12
echo ""
...
\"$SCRIPT_DIR/uv_env_manager.sh\" install demo-env numpy
echo ""

echo -e "${GREEN}6. 在环境中运行命令${NC}"
echo "uv-run demo-env python -c \"import numpy; print('NumPy version:', numpy.__version__)\""
\"$SCRIPT_DIR/uv_env_manager.sh\" run demo-env python -c \"import numpy; print('NumPy version:', numpy.__version__)\"
echo ""

echo -e "${GREEN}7. 查看环境信息${NC}"
echo "uv-info demo-env"
\"$SCRIPT_DIR/uv_env_manager.sh\" info demo-env
echo ""
...
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
\"$SCRIPT_DIR/uv_env_manager.sh\" remove demo-env
fi
echo ""
